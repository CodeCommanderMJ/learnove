...
const sendTokenResponse = async (user, statusCode, res) => {
  ...
  if (redisClient) {
    await redisClient.setEx(`refresh_token:${user._id}`, 30 * 24 * 60 * 60, refreshToken);
  }
...
export const refreshToken = async (req, res) => {
  try {
    ...
    if (redisClient) {
      await redisClient.setEx(`refresh_token:${user._id}`, 30 * 24 * 60 * 60, newRefreshToken);
    }
...
export const resendVerification = async (req, res) => {
  try {
    ...
    await sendEmail({
      to: user.email,
      subject: 'Email Verification - Learnova',
      template: 'emailVerification',
      data: {
        firstName: user.firstName,
        verificationToken,
